...
Input: `docs/test-scenarios.md` (54 scenarios, TC-001 to TC-511)
...
| **E2E** | 13 | Critical user journeys, multi-page flows, cross-session security | 2–5 min total |
| **Total** | **54** | | |

**Pyramid shape**: Unit (5) → API (22) → Component (14) → E2E (13) ✓
...
| TC-109 | "Clear all bookings" button visible even with zero bookings | `data.data = []` | Button rendered outside the `bookings.length > 0` branch — always in DOM |
...
| TC-511 | "Clear all bookings" uses native browser confirm (not custom modal) | Must use `page.on('dialog')` handler — native `window.confirm()` in `handleClearAll`; critical distinction from Cancel Booking which uses `<ConfirmDialog>` |
...
| Using `page.getByRole('dialog')` for "Clear all bookings" confirm | TC-511 | "Clear all" uses `window.confirm()` (native browser dialog) — must use `page.on('dialog', d => d.accept())`; "Cancel Booking" uses `<ConfirmDialog>` React component — use DOM locators |
...
- `TC-503, TC-506, TC-511` (cancel/clear dialogs + success — E2E)
