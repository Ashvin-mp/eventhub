...
### TC-109: Bookings list always shows "Clear all bookings" button (even with zero bookings)
**Category**: Business Rule
**Priority**: P2
**Preconditions**: User is logged in (zero or more bookings)
**Steps**:
1. Navigate to `/bookings` with no bookings
2. Look for "Clear all bookings" link
**Expected Results**: "Clear all bookings" link is always visible in the top-right regardless of booking count — it is not conditionally hidden; clicking it with no bookings triggers native browser confirm then no-ops
**Business Rule**: `BookingsContent` renders the button unconditionally (not inside the `bookings.length > 0` branch)
**Suggested Layer**: E2E / Component
...
### TC-511: "Clear all bookings" uses native browser confirm dialog (not custom modal)
**Category**: UI State
**Priority**: P1
**Preconditions**: User has bookings; user is on `/bookings`
**Steps**:
1. Click "Clear all bookings"
2. Observe the dialog type that appears
**Expected Results**: Native browser `window.confirm()` dialog appears with text "Clear all your bookings? This cannot be undone." — NOT the custom `ConfirmDialog` component used for single-booking cancellation
**Business Rule**: `handleClearAll` calls `window.confirm(...)` directly; cancel uses `<ConfirmDialog>` component — two different confirmation patterns
**Suggested Layer**: E2E
